...
class ProductView(View):
	def get(self, request):
		...
		jewelleries = Product.objects.filter(category='J')
		if request.user.is_authenticated:
			totalitem = len(Cart.objects.filter(user=request.user))
		return render(request, 'home.html', {'topwears':topwears, 'bottomwears':bottomwears, 'jewelleries':jewelleries, 'totalitem':totalitem})
...
def checkout(request):
	...
	return render(request, 'checkout.html', {'add':add, 'cart_items':cart_items})
...
def jewellery(request, data=None):
	...
	if data==None :
			jewelleries = Product.objects.filter(category='J')
	elif data == 'Ring' or data == 'Sted':
			jewelleries = Product.objects.filter(category='J').filter(brand=data)
	elif data == 'below':
			jewelleries = Product.objects.filter(category='J').filter(discounted_price__lt=500)
	elif data == 'above':
			jewelleries = Product.objects.filter(category='J').filter(discounted_price__gt=500)
	return render(request,'jewellery.html', {'jewelleries':jewelleries, 'totalitem':totalitem})
